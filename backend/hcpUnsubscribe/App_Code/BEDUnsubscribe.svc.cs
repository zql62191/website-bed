...
namespace BEDService
{
    ...
    public class BEDUnsubscribe : IUnsubscribeBED
    {
        ...
        public BEDUnsubscribe()
        {

        }

        public string TestUnsubscribe()
        {
            return "function called";
        }

        public string TestUnsubscribeString(string str)
        {
            return str;
        }

        public Status SetUnsubscribeDataEmail(FormEmail email, string sourceCode)
        {
            ...
            try
            {
                status.Errors = ValidateEmail(email);
        ...
        public Status SetUnsubscribeDataAddress(FormEmail email, FormAddress address, string sourceCode)
        {
            AuditTrail auditTrail = new AuditTrail();
            Status status = new Status();
            HCPIndividual hcp;
            ...
            try
            {
                status.Errors = ValidateAddress(address);
                status.Errors.AddRange(ValidateEmail(email));
                if (status.Errors.Count > 0)
                {
        ...
        public Status SetUnsubscribeDataBoth(FormEmail email, FormAddress address, string sourceCode)
         {
            AuditTrail auditTrail = new AuditTrail();
            Status status = new Status();
            HCPIndividual hcp;
